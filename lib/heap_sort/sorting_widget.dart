...
class InheritedSortingWidget extends InheritedWidget {
  ...
  InheritedSortingWidget({@required this.uiSortingDataProvider});
...
class _SortingWidgetState extends State<StatefulWidget> {
    ...
    return BlocListener<SortingBloc, SortingState>(
      listener: (context, state) {
        if (state is SwapingItemsSortingState) {
          swipeItems(state.index1, state.index2);
          return;
        }
      ...
      child:
          InheritedSortingWidget(uiSortingDataProvider: uiSortingDataProvider),
    );
  }
}
